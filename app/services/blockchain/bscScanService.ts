...
import { BlockchainTransaction, PaymentValidationResult, BscScanResponse } from '../../types/usdtPayment';
...
export class BscScanService {
  ...
  async getUSDTBalance(address: string): Promise<number> {
    try {
      ...
      console.error('Error fetching BscScan USDT balance:', error);
      throw error;
    }
  }
  ...
  async getUSDTTokenTransfers(address: string, limit: number = 50): Promise<BlockchainTransaction[]> {
    try {
      ...
      console.error('Error fetching BscScan USDT transactions:', error);
      throw error;
    }
  }
  ...
  async getTransactionInfo(txHash: string): Promise<BlockchainTransaction | null> {
    try {
      ...
      const receiptData: BscScanResponse = await receiptResponse.json();
      ...
      console.error('Error fetching BscScan transaction:', error);
      return null;
    }
  }
  ...
  async validatePayment(
    ...
    try {
      ...
      // Конвертируем сумму (для BEP-20 это обычно в hex формате)
      ...
      console.error('Error validating BscScan payment:', error);
      return {
        isValid: false,
